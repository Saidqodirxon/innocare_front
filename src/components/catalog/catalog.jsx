...
const Catalog = () => {
    ...
    axios
      ...
      .then(({ data }) => {
        ...
        const all = (data?.data || []).map((p) => ({
          ...p,
          title: i18n.language === "ru" ? p.name_ru : p.name_uz,
          img: p.image?.[0]?.url || "",
        }));
        setProducts(all);
      })
  ...
  return (
    <div className="py-12 px-4 md:px-16 bg-white" id="services">
      {/* RESET: agar loyihada .swiper-slide opacity/scale berilgan bo‘lsa, buni qoldiring */}
      <style>{`
        .catalog-swiper .swiper-slide,
        .catalog-swiper .swiper-slide-prev,
        .catalog-swiper .swiper-slide-next,
      ...
      <div className="relative max-w-[1450px] mx-auto catalog-swiper">
        {/* Prev / Next */}
        <button
          ref={prevRef}
          aria-label="Previous"
        ...
        >
          {products.map((item) => (
            <SwiperSlide key={item._id}>
              {/* Outer div — ichida ikki ta <Link> borligi uchun nesting <a> muammosini oldini olamiz */}
